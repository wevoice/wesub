...
if [ ! -z "$DOCKER_HOST" ] ; then
    ...
    #IPADDR="$DOCKER_HOST"
    IPADDR=`echo ${DOCKER_HOST} | awk -F  ":" '{print $1}'`
else
    DOCKER="docker"
...
# run tests
TEST_APP=$($DOCKER run -i -d -h unisubs.example.com -e AWS_ACCESS_ID=$AWS_ID -e AWS_SECRET_KEY=$AWS_KEY -e REVISION=$REV -e TEST_IPADDR=$IPADDR -e TEST_BROKER_PORT=$RABBITMQ_PORT -e TEST_SOLR_PORT=$SOLR_PORT -e TEST_MEMCACHED_PORT=$MEMCACHED_PORT -e DJANGO_LIVE_TEST_SERVER_ADDRESS='localhost:8090-8100,9000-9200' -e RUN_SELENIUM="$RUN_SELENIUM" amara-app /usr/local/bin/test_app)
echo "Test Container: $TEST_APP"
RETVAL=$($DOCKER wait $TEST_APP)
RETVAL=$?
# output test log
