...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "For threads with recent activity, set tip correctly"
        for thread in set(Message.objects.filter(created__gt=datetime.datetime.now() - datetime.timedelta(days=30), thread__isnull=False).values_list('thread', flat=True)):
