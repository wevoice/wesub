...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm['messages.Message'].objects.all().filter(message_type='O', author__isnull=True).update(message_type='S')
        orm['messages.message'].objects.all().filter(message_type='O').update(message_type='M')
        # This should not exist, unless migrations are run back and forth:
        orm['messages.Message'].objects.all().filter(message_type__isnull=True, author__isnull=True).update(message_type='S')
        orm['messages.message'].objects.all().filter(message_type__isnull=True).update(message_type='M')
        orm['messages.Message'].objects.all().filter(message_type='', author__isnull=True).update(message_type='S')
        orm['messages.message'].objects.all().filter(message_type='').update(message_type='M')
