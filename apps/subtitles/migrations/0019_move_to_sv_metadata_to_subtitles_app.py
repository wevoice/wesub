...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        OldSubtitleVersionMetadata = orm['videos.SubtitleVersionMetadata']
        NewSubtitleVersionMetadata = orm['subtitles.SubtitleVersionMetadata']

        migrated = 0
        already_there = 0
        missing_from_tern = 0
        counter = -1
        total = OldSubtitleVersionMetadata.objects.count() * 1.0
        for old_meta in chunkediter(OldSubtitleVersionMetadata.objects.all()):
            print "Running sv metadata %s / %s (%s)" % (counter, total, counter/total)
            counter +=1
            new_subtitle_version = old_meta.subtitle_version.new_subtitle_version
            if not new_subtitle_version:
                print "ERROR: svmetadata %s for subtitle version: %s has not been migrated" % \
                      ( old_meta.pk, old_meta.subtitle_version.pk)
                missing_from_tern += 1
                continue
            try:
                NewSubtitleVersionMetadata.objects.get(
                    subtitle_version=new_subtitle_version,
                    key=old_meta.key
                )
                already_there += 1
            except models.ObjectDoesNotExist:
                new_meta = NewSubtitleVersionMetadata.objects.create(
                    key=old_meta.key,
                    subtitle_version=new_subtitle_version,
                    data=old_meta.data,
                )
                # you can't use the regular save method, as the datetime
                # stamps are auto created and will be from the time the
                # migration has run, qs.update won't trigger this, yuck:
                NewSubtitleVersionMetadata.objects.filter(pk=new_meta.pk).update(
                    created=old_meta.created,
                    modified=old_meta.modified,
                )
                migrated +=1
        print "DONE:"
        print " migrated:          %s" % migrated
        print " already present:   %s" % already_there
        print " missing from tern: %s" % missing_from_tern
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 4, 23, 9, 54, 22, 596453)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 4, 23, 9, 54, 22, 596322)'}),
    ...
    complete_apps = ['videos', 'subtitles']
