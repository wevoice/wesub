...
class Migration(SchemaMigration):
    tables = [
        'externalsites_kalturaaccount',
        'externalsites_brightcoveaccount',
    ]
    ...
    def forwards(self, orm):
        for table_name in self.tables:
            # add new columns
            db.add_column(table_name, 'type', self.gf('django.db.models.fields.CharField')(default='T', max_length=1), keep_default=False)
            db.add_column(table_name, 'owner_id', self.gf('django.db.models.fields.IntegerField')(default=0), keep_default=False)
            
            # move data from team_id to owner_id
            db.execute("UPDATE %s SET type='T', owner_id=team_id" %
                       table_name)
            # delete the team_id column
            db.delete_column(table_name, 'team_id')
            # create unique constraint
            db.create_unique(table_name, ['type', 'owner_id'])

    
    def backwards(self, orm):
        for table_name in self.tables:
            # add back the team_id column
            db.add_column(table_name, 'team', self.gf('django.db.models.fields.related.OneToOneField')(default=None, to=orm['teams.Team'], unique=True), keep_default=False)
            # delete non-team accounts
            db.execute("DELETE FROM %s WHERE type != 'T'" % table_name)
            # move data from owner_id to team_id
            db.execute("UPDATE %s SET team_id=owner_id" % table_name)
            # Delete fields
            db.delete_column(table_name, 'type')
            db.delete_column(table_name, 'owner_id')
            #db.delete_unique(table_name, ['type', 'owner_id'])
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 6, 4, 8, 54, 24, 515457)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 6, 4, 8, 54, 24, 515383)'}),
