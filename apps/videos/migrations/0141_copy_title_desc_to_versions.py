...
def latest_version(language, public_only=True):
    try:
        return _filter_public(language, language.subtitleversion_set.all(), public_only)[0]
    except IndexError:
        return None
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            versions = orm.SubtitleVersion.objects.select_related(
                'language', 'language__video', 'language__standard_language').iterator()

            for version in versions:
                language = version.language
                orig_language = language.standard_language
                video = language.video

                stripped = lambda s: s.strip() if s else None

                if stripped(language.title):
                    title = stripped(language.title)
                elif orig_language and stripped(orig_language.title):
                    title = stripped(orig_language.title)
                elif stripped(video.title):
                    title = stripped(video.title)
                else:
                    title = ''

                if stripped(language.description):
                    description = stripped(language.description)
                elif orig_language and stripped(orig_language.description):
                    description = stripped(orig_language.description)
                elif stripped(video.description):
                    description = stripped(video.description)
                else:
                    description = ''
    ...
    def backwards(self, orm):
        if not db.dry_run:
            languages = orm.SubtitleLanguage.objects.iterator()

            for language in languages:
                version = latest_version(language, public_only=True)

                if not version:
                    version = latest_version(language, public_only=False)

                if not version:
                    continue

                language.title = version.title
                language.description = version.description
                language.save()
