...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # We should always set Video.title to the SubtitleVersion.title when
        # we get a version is published for the primary audio language.
        # However, there was a corner case with tasks where this wasn't
        # happening.  This migration fixes the video titles for that case.
        #
        # Note: Writing the update in 1 statement makes the SQL crazy, so we
        # select into a temprorary table then do the update.

        db.execute("""\
CREATE TEMPORARY TABLE new_video_titles AS (
SELECT v.id, sv.title
FROM videos_video v
JOIN subtitles_subtitleversion sv
ON sv.video_id=v.id AND
   sv.language_code = v.primary_audio_language_code AND
   sv.version_number = (
      SELECT MAX(sv2.version_number)
      FROM subtitles_subtitleversion sv2
      WHERE sv2.subtitle_language_id = sv.subtitle_language_id
      AND ((sv2.visibility = 'public' AND sv2.visibility_override = '') OR
              sv2.visibility_override = 'public'))
WHERE v.title != sv.title and v.title != '' AND sv.title != '')""")
        db.execute("""\
UPDATE videos_video
JOIN new_video_titles
ON videos_video.id = new_video_titles.id
SET videos_video.title = new_video_titles.title""")
        db.execute("DROP TABLE new_video_titles")
    ...
    models = {
        ...
        'videos.videourl': {
            'Meta': {'ordering': "('video', '-primary')", 'unique_together': "(('url', 'type'),)", 'object_name': 'VideoUrl'},
