...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Video = orm['videos.video']
        videos = Video.objects.annotate(team_video_count=models.Count('teamvideo')).filter(team_video_count__gt=1)

        print len(videos), 'videos with duplicate team videos.'

        with open('/tmp/duplicate_team_video_removal_%d.log' % int(time.time()), 'w') as f:
            for video in videos:
                sys.stdout.write('.')
                team_videos = list(video.teamvideo_set.all())
                team_videos.sort(key=lambda tv: tv.created)
                msg = "Video %s has %d team videos: %s\n" % (
                        video.video_id,
                        len(team_videos),
                        ' '.join('tv-%d/team-%d' % (tv.id, tv.team_id) for tv in team_videos))
                f.write(msg)

                keep = team_videos[0]
                kill = team_videos[1:]

                msg = "Keeping %s, deleting: %s\n" % (
                        keep.id,
                        ' '.join(str(tv.id) for tv in kill))
